...
### read all sap files
sap_files = ["DataMAKT","DataMARA", "DataMARC"]
...
for i in range(0,len(sap_files)):
    sap_file = os.path.join(inDir, 'MATERIAL SAP/' + sap_files[i]+'.xlsx')
    # print(sap_file)
...
# match_vals = nfm.fuzzyWordMatch(legacy_data_clean, 'legacy', sap_fields, 'sap', 'Comments', 'Description', 'Name', 'Field', outFile, 3)
